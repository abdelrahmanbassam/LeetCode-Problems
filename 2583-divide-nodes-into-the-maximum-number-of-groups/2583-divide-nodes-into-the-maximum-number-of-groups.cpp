...
public:
    ...
    int componentDFS(vector<vector<int>> &adjList, int node, vector<int> &distances, vector<bool> &visited) {
        int maxNumberOfGroups = distances[node];
        visited[node] = true;

        for (int neighbor : adjList[node]) {
            if (visited[neighbor]) continue;
            maxNumberOfGroups = max(maxNumberOfGroups,componentDFS(adjList, neighbor, distances, visited));
        }
    ...
    int magnificentSets(int n, vector<vector<int>> &edges) {
        ...
        for (int node = 0; node < n; node++) {
            ...
            maxNumberOfGroups += componentDFS(adjList, node, distances, visited);
        }
